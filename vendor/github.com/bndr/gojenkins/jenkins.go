...
func (j *Jenkins) CreateFolderInView(ctx context.Context, name string, view string, parents ...string) (*Folder, error) {
	folderObj := &Folder{Jenkins: j, Raw: new(FolderResponse), Base: "/view/" + view + "/job/" + strings.Join(append(parents, name), "/job/")}
	folder, err := folderObj.Create(ctx, name)
	if err != nil {
		return nil, err
	}
